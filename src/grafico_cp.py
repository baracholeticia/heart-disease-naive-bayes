...
def load_prepared_data():
    ...
    if not os.path.exists(train_path):
        raise FileNotFoundError('data/train.csv não encontrado')
...
def main():
    ...
    bars0 = ax.bar(x - width / 2, probs_y0, width, label='Y=0 (sem doença)', color='#4C72B0')
    bars1 = ax.bar(x + width / 2, probs_y1, width, label='Y=1 (com doença)', color='#C44E52')
