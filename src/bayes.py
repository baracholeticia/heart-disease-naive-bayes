...
def explain_example(x, feature_model, priors: dict, feature_name: str) -> str:
    ...
    verossimilhancas = [f'  p({feature_name}={x} | Y={c}) = {likelihood_fn(x, c):.5f}   (verossimilhança)' for c in sorted(priors)]
    posteriores = [f'  P(Y={c} | {feature_name}={x}) = {post[c]:.5f}   (probabilidade da classe após considerar a observação)' for c in sorted(priors)]
    pred = max(post, key=post.get)
    decisao = f'  **Decisão Bayesiana: Y = {pred}** (maior probabilidade a posteriori)'
    linhas = [f'Exemplo numérico para {feature_name} = {x}', *verossimilhancas, *posteriores, decisao]
    return '\n'.join(linhas)

def find_decision_boundaries(feature_model, priors: dict, x_min: float, x_max: float, resolution: int=20000) -> list:
    xs = np.linspace(x_min, x_max, resolution)
    diffs = np.array([feature_model.pdf(x, 1) * priors[1] - feature_model.pdf(x, 0) * priors[0] for x in xs])
